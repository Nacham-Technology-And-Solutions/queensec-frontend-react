...
const VendorTicketStatusByNumberPlate = () => {
  ...
  const numberPlateParam = queryParams.get('number_plate');
  ...
  useEffect(() => {
    const fetchPaymentDetails = async () => {
      ...
      try {
        const orderResponse = await axios.post(`${API_BASE_URL}/wallet/ticket-status`, {
          'number_plate': numberPlateParam
        }, {
          headers: {
  ...
  const setCurrentTicket = (index) => {
    ...
    setAmount(selectedTicket.amount || '0');
    setHaulerTypeName(selectedTicket.hauler_type_name || '');
    setTransactionId(selectedTicket.wallet_transaction_id || '');
    setNumberPlate(selectedTicket.number_plate || '');
    setMineralName(selectedTicket.mineral_name || '');
    setMineralSymbol(selectedTicket.mineral_symbol || '');
    setDate(new Date(selectedTicket.date).toLocaleDateString());
    setTime(new Date(selectedTicket.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }));
    setTicketId(selectedTicket.ticket_id || '');
    setStatus(selectedTicket.status || '');
    setStatusMessage(selectedTicket.status_message || '');
    setScanned(selectedTicket.scanned || '');
  ...
  return (
    <Container>
      ...
      <TopBar>
        ...
        <Title>Ticket Status By Number Plate</Title>
      </TopBar>

      {(tickets.length === 0) && (<p>No Ticket Found</p>)}


      {(tickets.length > 0 && activeTicket < 0) &&
        (<>

          {tickets.map((ticket, index) => (
            <TransactionItem onClick={() => setCurrentTicket(index)} key={ticket.wallet_ticket_order_id}>
              <IconContainer>{ticket.mineral_symbol}</IconContainer>
              <TransactionDetails>
                <ItemTitle>{ticket.mineral_name}</ItemTitle>
                <ItemCode>{ticket.number_plate}</ItemCode>
              </TransactionDetails>
              <AmountContainer>
                <AmountToday>₦{ticket.amount}</AmountToday>
                <DateText>{new Date(ticket.date).toLocaleDateString()} - {new Date(ticket.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}</DateText>
              </AmountContainer>
            </TransactionItem>
...
export default VendorTicketStatusByNumberPlate;
